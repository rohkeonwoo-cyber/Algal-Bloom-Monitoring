      ...
      void main(){
        ...
        if(uHasDetail > 0.5){
          ...
          if(d.x > 0.0 && d.x < 1.0 && d.y > 0.0 && d.y < 1.0){
            ...
            vec4 dt = texture2D(uDetail, d);   // 알파 0 = 아직 안 온 타일 → 밑 텍스처 유지
            c = mix(c, dt.rgb, dt.a * smoothstep(0.0, 0.02, e));
          }
        }
...
async function refreshDetail(){
  ...
  // 빈 칸은 투명으로 둔다. 예전에는 올리브색으로 칠해, 타일이 늦거나 실패한 자리가
  // 산 위의 연두색 네모로 박혀 보였다. 투명이면 셰이더가 밑의 위성 텍스처를 그대로 보여준다.
